...
const std::string MORSE_TABLE[128]{
    "",		// 0	NUL
    "",		// 1	SOH
    "",		// 2	STX
    "",		// 3	ETX
...
{
    // std::cout << (int)c << " " ;
    if (MORSE_TABLE[(int)c] != "")
    {
        // std::cout << c << " = " << MORSE_TABLE[(int)c] << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    struct option long_options[] =
    {
        ...
        {"to_morse",            no_argument,       0, 'm'},
        {0,         0,                 0,  0 }
    };

    // Parse options using getopt_long
    while ((opt = getopt_long(argc, argv, "hvisotm", long_options, nullptr)) != -1)
    {
        switch (opt)
        {
            case 'h':
                std::cout << "Usage: mctranslator [options] SOURCE DEST\n"
                          ...
                          << "  -m  --to_morse Translates text into morse code\n"
                          << "  -t  --to_text  Translates morse code into text";
                return 0;
            case 'v':
                verbose = true;
                break;
        ...
        if (inputFile.is_open() && outputFile.is_open())
        {
            ...
            while ( getline (inputFile,line) )
            {
                ...
                while ( getline (ss,letter, ' ') )
                {
                    if (verbose)
                    {
                        std::cout << morseToText(letter);
                    }
                    outputFile << morseToText(letter);
                }
                ss.clear();
                ss.str(std::string());
